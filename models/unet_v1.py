...
def conv_conv_pool(input_, n_filters, mode, hyper_params, name, pool=True, activation=tf.nn.relu):
...
def upconv_concat(inputA, input_B, n_filter, hyper_params, name):
	
	up_conv = upconv_2D(inputA, n_filter, hyper_params, name)
	return tf.concat([up_conv, input_B], axis=-1, name="concat_{}".format(name))

def upconv_2D(tensor, n_filter, hyper_params, name):
...
def description(features, labels, mode, params, config):
	...
	conv1, pool1 = conv_conv_pool(input_data, [32, 32], mode, hyper_params, name=1)
	conv2, pool2 = conv_conv_pool(pool1, [64, 64], mode, hyper_params, name=2)
	conv3, pool3 = conv_conv_pool(pool2, [128, 128], mode, hyper_params, name=3)
	conv4 = conv_conv_pool(pool3, [256, 256], mode, hyper_params, name=4, pool=False)

	up7 = upconv_concat(conv4, conv3, 128, hyper_params, name=7)
	conv7 = conv_conv_pool(up7, [128, 128], mode, hyper_params, name=7, pool=False)

	up8 = upconv_concat(conv7, conv2, 64, hyper_params, name=8)
	conv8 = conv_conv_pool(up8, [64, 64], mode, hyper_params, name=8, pool=False)

	up9 = upconv_concat(conv8, conv1, 32, hyper_params, name=9)
	conv9 = conv_conv_pool(up9, [32, 32], mode, hyper_params, name=9, pool=False)
